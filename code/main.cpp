...
int main(void) {
    ...
    for(;;)
    {
        ...
        float voltage = adc_get_sys_voltage();
        unsigned long batt = voltage * 1000;
        chnWriteTimeout(&nrf24l01.channels[0], (uint8_t*)&batt, sizeof(unsigned long), MS2ST(250));
        if (res > 100)
        {
            res = 0;
        }
        ...
        if (voltage < 3.4)
        {
            ws2812b_set_led_color(0, ((uint32_t)(128 + 3 * vector.y) << 16));
            ws2812b_set_led_color(1, ((uint32_t)(128 - 3 * vector.x) << 16));
            ws2812b_set_led_color(2, ((uint32_t)(128 - 3 * vector.y) << 16));
            ws2812b_set_led_color(3, ((uint32_t)(128 + 3 * vector.x) << 16));
        }
        else
        {
